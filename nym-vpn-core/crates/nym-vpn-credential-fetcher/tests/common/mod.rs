...
use nym_vpn_api_client::{
    ...
    response::{
        NymVpnHealthResponse, NymVpnZkNym, NymVpnZkNymPost, NymVpnZkNymResponse,
        NymVpnZkNymStatus, StatusOk,
    },
};
...
impl MockZkNymApi {
                ...
                Mock::given(method("GET"))
                    ...
                    .respond_with(
                        ResponseTemplate::new(200)
                            .set_body_json(self.partial_verification_keys()),
                    ),
            )
            .await;
        server
    ...
    fn poll_zknym(self) -> impl Respond {
        move |req: &Request| {
            ...
            let shares = (record.status == NymVpnZkNymStatus::Active)
                .then(|| self.issue_wallet_shares(clone_request(&record.request)).unwrap());
    ...
    fn list_zknyms(self) -> impl Respond {
        move |_req: &Request| {
            ...
            let items = state
                ...
                .iter()
                .map(|(id, record)| {
                    zknym_response(id, &record.request.ticketbook_type, record.status.clone(), None)
                })
                .collect::<Vec<_>>();
