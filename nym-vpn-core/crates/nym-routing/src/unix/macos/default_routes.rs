...
impl DefaultRouteMonitor {
    ...
    pub fn start(
        ...
        let (route_v4_tx, route_v4_rx) = mpsc::unbounded_channel();
        let (route_v6_tx, route_v6_rx) = mpsc::unbounded_channel();
    ...
    async fn run(mut self) {
        ...
        while let Some(events) = self.event_rx.recv().await {
...
fn filter_duplicates<T: PartialEq + Clone + Send + 'static>(
    ...
    async fn do_filtering<T: PartialEq + Clone + Send + 'static>(
        ...
        let mut last_value = unfiltered_rx.recv().await?;
        filtered_tx.send(last_value.clone()).ok()?;

        loop {
            let prev_value = mem::replace(&mut last_value, unfiltered_rx.recv().await?);
