...
impl RoutingTable {
    ...
    pub async fn add_route(&mut self, message: &RouteMessage) -> Result<AddResult> {
        ...
        tracing::trace!("Add route: {message:?}");
    ...
    pub async fn delete_route(&mut self, message: &RouteMessage) -> Result<()> {
        tracing::trace!("Delete route: {message:?}");
    ...
    pub fn dump_routes(&self, address_family: std::ffi::c_int) -> Result<Vec<data::RouteMessage>> {
        let mut mib: [std::ffi::c_int; 6] = [
            libc::CTL_NET,
            libc::PF_ROUTE,
            0,
            address_family,
        ...
        while offset + std::mem::size_of::<libc::rt_msghdr>() <= buffer.len() {
            ...
            if msg_len < std::mem::size_of::<libc::rt_msghdr>() || offset + msg_len > buffer.len() {
                break;
            }
