...
A SOCKS5 proxy that selectively routes outbound connections either through the Nym VPN tunnel or directly via the default network interface, based on GeoIP data and domain exclusion lists. This is used to implement the Airporting feature, which allows certain traffic (e.g. traffic destined for excluded country IP ranges) to bypass the VPN.
