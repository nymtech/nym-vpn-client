...
impl LazyMetadataClient {
    async fn new(
        base_url: Url,
        rx_channel: tokio::sync::oneshot::Receiver<IpAddr>,
    ) -> Result<Self> {
        let reqwest_builder = ReqwestClientBuilder::new().local_address(rx_channel.await?);
...
pub struct MetadataClient {
    ...
    base_url: Url,
    // this field should only be used inside lazy_client, that consumes it only once
    rx_channel: Option<tokio::sync::oneshot::Receiver<IpAddr>>,
}
        ...
        self.lazy_client
            .get_or_init(|| async {
                LazyMetadataClient::new(
                    self.base_url.clone(),
                    self.rx_channel.take().ok_or(MetadataClientError::Internal(
                        "rx_channel can't be constructed as None".to_string(),
                    ))?,
                )
                .await
            })
