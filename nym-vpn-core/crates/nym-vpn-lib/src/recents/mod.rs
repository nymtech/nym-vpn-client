...
use crate::recents::{error::RecentsError, gateway_cache::GatewayCache};
...
#[derive(Clone)]
pub struct RecentsManager<C: GatewayCache> {
    file_path: PathBuf,
    gateway_cache: C,
...
impl<C: GatewayCache> RecentsManager<C> {
    pub async fn new(dir_path: PathBuf, gateway_cache: C) -> Self {
        ...
        match persisted_recents(&file_path).await {
            ...
            None => {
                let cache = Arc::new(RwLock::new(Recents::default()));
                let recents_manager = Self {
                    file_path,
                    gateway_cache,
                    cache,
                };
                // write the first empty recents, to have the file created on disk
                recents_manager.flush().await;
                recents_manager
            }
        }
    }
    ...
    async fn flush(&self) {
        let Ok(contents) = self.cache.read().await.to_bytes() else {
            tracing::warn!("Could not serialize recents cache");
            return;
        };
        if let Err(err) = tokio::fs::write(&self.file_path, contents).await {
    ...
    pub async fn add_recent(&mut self, gateway_type: TunnelType, entry: String, exit: String) {
        {
            let mut cache: tokio::sync::RwLockWriteGuard<'_, Recents> = self.cache.write().await;
            let inner = match gateway_type {
    ...
    pub async fn get_recent(
        &self,
        gateway_type: TunnelType,
    ) -> Result<RecentGateways, RecentsError> {
        let cache = self.cache.read().await.clone();
        let (inner, entry_gateways, exit_gateways) = match gateway_type {
            TunnelType::Mixnet => (
        ...
        let recent_entry = Self::get_recent_queue(&inner.entry, &entry_gateways);
        let recent_exit = Self::get_recent_queue(&inner.exit, &exit_gateways);
        Ok(RecentGateways {
            entry: recent_entry.into(),
            exit: recent_exit.into(),
        })
    }
}
