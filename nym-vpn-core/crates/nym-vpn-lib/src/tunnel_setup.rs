...
pub use select_gateways::SelectGatewaysError;
...
pub(crate) async fn setup_tunnel(
    ...
    let select_gateways::SelectedGateways { entry, exit } =
        select_gateways::select_gateways(&gateway_directory_client, nym_vpn).await?;
