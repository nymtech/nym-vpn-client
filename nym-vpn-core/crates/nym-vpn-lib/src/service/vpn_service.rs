...
impl NymVpnService {
    ...
    async fn handle_service_command(&mut self, command: VpnServiceCommand) {
        match command {
            ...
            VpnServiceCommand::SetGeoexclusionEnabled(tx, enabled) => {
                self.handle_set_geoexclusion_enabled(enabled).await;
                let _ = tx.send(());
            }
            VpnServiceCommand::SetGeoexclusionListenPort(tx, listen_port) => {
                self.handle_set_geoexclusion_listen_port(listen_port).await;
                let _ = tx.send(());
            }
            VpnServiceCommand::SetGeoexclusionExcludedCountries(tx, excluded_countries) => {
                let result = self
                    .handle_set_geoexclusion_excluded_countries(excluded_countries)
                    .await;
                let _ = tx.send(result);
            }
        }
    ...
    async fn handle_set_geoexclusion_enabled(&mut self, enabled: bool) {
        self.config_manager.set_geoexclusion_enabled(enabled).await;
        self.update_tunnel_settings_with_throttle();
    }

    async fn handle_set_geoexclusion_listen_port(&mut self, listen_port: u16) {
        self.config_manager
            .set_geoexclusion_listen_port(listen_port)
            .await;
        self.update_tunnel_settings_with_throttle();
    }

    async fn handle_set_geoexclusion_excluded_countries(
        &mut self,
        excluded_countries: Vec<String>,
    ) -> Result<(), GeoexclusionConfigError> {
        self.config_manager
            .set_geoexclusion_excluded_countries(excluded_countries)
            .await?;
        self.update_tunnel_settings_with_throttle();
        Ok(())
    }
