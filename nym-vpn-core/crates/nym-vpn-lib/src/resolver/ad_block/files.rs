...
pub(crate) static AD_BLOCKING_LIST_DESCRIPTORS: &[AdBlockListDescriptor] = &[
    AdBlockListDescriptor {
...
static USER_AGENT: &str = "ad-blocker-lists/1.0";
...
pub(crate) async fn init_files(data_dir: &Path, force: bool) -> Result<()> {
    ...
    for descr in AD_BLOCKING_LIST_DESCRIPTORS.iter() {
        descr.init(&ad_blocking_path, force).await?;
    }
...
pub(crate) async fn load_filter_set(data_dir: &Path) -> Result<FilterSet> {
    ...
    for descr in AD_BLOCKING_LIST_DESCRIPTORS.iter() {
        let meta_path = ad_blocking_path.join(descr.meta_file_name);
        let meta_data = AdBlockListMeta::from_file(&meta_path).await?;
        let domain_list = descr.load_data_file(&ad_blocking_path, &meta_data).await?;
        filter_set.add_filter_list(
            &domain_list,
            ParseOptions {
                format: descr.filterset_format,
...
pub(crate) struct AdBlockListDescriptor {
...
impl AdBlockListDescriptor {
    ...
    /// Returns Ok(true) if the file was updated, Ok(false) if it was already up to date, or an error if the update failed.
    async fn update_data_file(
        &self,
        ad_locking_path: &Path,
        http_client: &reqwest::Client,
    ) -> Result<bool> {
        // Read the current meta file
        let meta_path = ad_locking_path.join(self.meta_file_name);
        let mut meta_data = AdBlockListMeta::from_file(&meta_path).await?;
        ...
        let request = http_client
            ...
            .header(reqwest::header::ACCEPT, "text/plain,*/*;q=0.9")
        ...
        let etag = response
            ...
            .ok_or(AdBlockingError::MissingEtag {
                url: self.url.to_string(),
            })?
            .to_str()
            .map_err(|error| AdBlockingError::InvalidEtag {
                url: self.url.to_string(),
                error,
            })?
            .to_string();
        ...
        let encoder = XzEncoder::new(data_vec, 9);
        let compressed_bytes = encoder
            .finish()
            .map_err(|error| AdBlockingError::CompressData { error })?;

        // Write the data file to a temporary file
        let temp_data_path = ad_locking_path.join("temp_data");
        fs::write(&temp_data_path, &compressed_bytes)
            .await
            .map_err(|error| AdBlockingError::WriteFile {
        ...
        // Update the meta data and write it to a temporary file
        meta_data.updated_from_website_utc = OffsetDateTime::now_utc();
        meta_data.bytes = data_bytes.len();
        meta_data.sha256 = format!("{:x}", Sha256::digest(&data_bytes));
        meta_data.etag = etag;

        let temp_meta_path = ad_locking_path.join("temp_meta");
        meta_data.write_to_file(&temp_meta_path).await?;

        // Now rename the temporary files to their final names, replacing the old ones
        let data_path = ad_locking_path.join(self.file_name);
    ...
    async fn load_data_file(
        ...
        decoder
            ...
            .map_err(|error| AdBlockingError::DecompressData { error })?;
...
pub(crate) struct AdBlockListMeta {
