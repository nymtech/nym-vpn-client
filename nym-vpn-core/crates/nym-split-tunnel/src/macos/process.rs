...
async fn handle_eslogger_output(
    ...
    tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = lines.next_line().await {
            ...
            let val: ESMessage = match serde_json::from_str(&line) {
                Ok(val) => val,
                Err(error) => {
                    tracing::error!("Failed to parse eslogger message: {error}");
                    continue;
                }
            };
    ...
    let result = tokio::select! {
            ...
            match result {
                Ok(status) => {
                    if let Ok(Some(last_error)) = last_stderr.await {
                        tracing::error!("eslogger error: {last_error}");
        ...
        Ok(response_tx) = stop_rx => {
            if let Err(error) = proc.kill().await {
                tracing::error!("Failed to kill eslogger: {error}");
            }
            if tokio::time::timeout(SHUTDOWN_TIMEOUT, proc.wait())
                .await
                .is_err()
            {
                tracing::error!("Failed to wait for ST process handler");
            }
    ...
    tracing::debug!("Process monitor stopped");
