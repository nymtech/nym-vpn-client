...
use nym_routing::{MacAddress, RouteManagerHandle};
...
pub enum Error {
    ...
    GetDefaultRoutes(#[source] nym_routing::Error),
    /// Failed to get default gateways
    #[error("Failed to get default gateways")]
    GetDefaultGateways(#[source] nym_routing::Error),
...
pub async fn get_default_interface(
    ...
    let default_v4 = if let Some(v4_gateway) = v4_gateway {
        ...
        tracing::debug!("Missing V4 gateway");
        None
    };
    let default_v6 = if let Some(v6_gateway) = v6_gateway {
        ...
        tracing::debug!("Missing V6 gateway");
        None
    };
