...
use super::windows::{
    ProcessAccess, get_device_path, get_overlapped_result, get_process_creation_time,
    get_process_device_path, open_process,
};
use bitflags::bitflags;
use memoffset::offset_of;
use nym_common::ErrorExt;
use nym_windows::{io::Overlapped, process::ProcessSnapshot, sync::Event};
use std::{
    cell::RefCell,
    collections::HashMap,
    ffi::{OsStr, OsString},
...
const DRIVER_SYMBOLIC_NAME: &str = r#"\\.\NYMVPNSPLITTUNNEL"#;
...
impl DeviceHandle {
    ...
    pub(super) fn new_handle_only() -> Result<Self, DeviceHandleError> {
        tracing::debug!("Connecting to the driver");
    ...
    pub fn reinitialize(&self) -> Result<(), DeviceHandleError> {
        ...
        if state != DriverState::Started {
            tracing::debug!("Resetting driver state");
            self.reset().map_err(DeviceHandleError::ResetError)?;
        }

        tracing::debug!("Initializing driver");
        self.initialize()
            .map_err(DeviceHandleError::InitializationError)?;

        tracing::debug!("Initializing driver process tree");
    ...
    pub fn set_config<T: AsRef<OsStr>>(&self, apps: &[T], hybrid_apps: &[T]) -> io::Result<()> {
        ...
        tracing::debug!("Excluded device paths:");
            ...
            Ok((
                ...
                EventBody::SplittingEvent {
                    ...
                    reason: SplittingChangeReason::from_bits(event.reason).unwrap_or_else(|| {
                        tracing::error!("Dropping unknown bits from splitting change reason. Original reason: {:b}", event.reason);
...
pub fn device_io_control(
    ...
    let out_buffer = if output_size > 0 {
        buffer.resize(output_size as usize, 0u8);
        Some(&mut buffer[..])
    } else {
        None
    };
...
pub fn device_io_control_buffer(
    ...
    get_overlapped_result(device.as_raw_handle(), overlapped)
}
