...
macro_rules! get_reparse_path {
    ($tag_type:ident, $data:ident) => {{
        ...
        if last_offset > $data.len() {
            tracing::error!("Ignoring mount point with out-of-bounds index");
            Err(io::Error::new(
...
fn resolve_link<P: AsRef<Path> + Copy>(path: P) -> io::Result<Option<PathBuf>> {
...
fn resolve_all_links_multiple<P: AsRef<Path>>(paths: &[P]) -> HashSet<PathBuf> {
    ...
    for path in paths {
        match resolve_all_links(path) {
            ...
            Err(error) => {
                tracing::error!("Failed to identify paths to monitor: {:?}", error);
            }
        }
    }
    monitored_paths
...
impl Drop for DirContext {
    fn drop(&mut self) {
        if let Err(error) = self.cancel_io() {
            tracing::error!("Failed to cancel pending file I/O request: {}", error);
        }
    }
}
...
impl PathMonitor {
        ...
        std::thread::spawn(move || {
            loop {
                ...
                match monitor.handle_next_completion_packet() {
                    ...
                    Ok(false) => (),
                    Err(error) => {
                        tracing::error!("handle_next_completion_packet failed: {}", error);
                        break;
                    }
                }
            }
            tracing::debug!("Shutting down reparse point monitor");
    ...
    fn update_paths(&mut self, unresolved_paths: &[PathBuf], force: bool) -> Result<bool, ()> {
        ...
        if force || new_stripped_paths != self.stripped_paths {
            ...
            if let Err(error) = self.update_directory_contexts() {
                tracing::error!("Failed to open new directory handles: {}", error);
                return Err(());
            }
            return Ok(true);
        }
        ...
        for i in (0..len).rev() {
                ...
                .any(|p| p.prefix == self.dir_contexts[i].path)
            {
                ...
                match removed_ctx.cancel_io() {
                    ...
                    Err(error) => {
                        tracing::error!("Failed to cancel pending I/O for dir context: {}", error);
    ...
    fn handle_next_completion_packet(&mut self) -> io::Result<bool> {
        let result = match self.port_handle.get_queued_completion_status() {
            ...
            Err((error, status)) => {
                ...
                if error.raw_os_error() != Some(ERROR_OPERATION_ABORTED as i32) {
                    tracing::error!("GetQueuedCompletionStatus failed: {:?}", error);
                    return Err(error);
                }
                return Ok(false);
            }
        ...
        let changed = if result.bytes_returned == 0 {
            tracing::trace!("Change event buffer is empty");
            false
        } else {
        ...
        if let Err(error) = self.dir_contexts[ctx_index].read_directory_changes() {
            tracing::error!("Failed to queue new directory change event: {}", error);
            return Err(error);
        }
        ...
        loop {
            ...
            for path in &self.stripped_paths {
                ...
                match cmp_status {
                    ...
                    0 => tracing::error!("Bug: CompareStringOrdinal failed"),
                    _ => (),
                }
            }
    ...
    fn abort_all_requests(&mut self) {
            ...
            .chain(self.discarded_contexts.drain(..))
        {
            match ctx.cancel_io() {
                ...
                Ok(false) => (),
                Err(error) => {
                    tracing::error!("Failed to cancel pending I/O request: {}", error);
                    mem::forget(ctx);
                }
            }
        }
        ...
        while !contexts.is_empty() {
            if time.elapsed() >= SHUTDOWN_TIMEOUT {
                tracing::error!("Timeout while cancelling I/O requests");
                mem::forget(contexts);
                return;
            }
