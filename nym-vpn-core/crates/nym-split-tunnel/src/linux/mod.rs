...
impl PidManager {
        ...
        return Self::new_cgroup2()
            ...
            .or_else(|cgroup2_err| {
                ...
                Self::new_cgroup1()
                    .map(Inner::CGroup1)
                    .map_err(|cgroup1_err| {
                        tracing::error!("Failed to initialize split-tunneling");
                        tracing::trace!("{cgroup1_err:?}");
                        tracing::trace!("{cgroup2_err:?}");
                        cgroup2_err
                    })
            });
    ...
    pub fn add(&mut self, pid: pid_t) -> Result<(), Error> {
        let pid = Pid::from_raw(pid);
        self.inner_mut()?.add(pid)
    }

    /// Remove a PID from the cgroup2 to have it included in the tunnel.
    pub fn remove(&mut self, pid: pid_t) -> Result<(), Error> {
        let pid = Pid::from_raw(pid);
        self.inner_mut()?.remove(pid)
    }
    ...
    /// Return true if split tunnel is supported, otherwise false.
    ...
    fn inner(&self) -> Result<&Inner, Error> {
        self.inner.as_ref().map_err(|_| Error::Unavailable)
    }

    fn inner_mut(&mut self) -> Result<&mut Inner, Error> {
        self.inner.as_mut().map_err(|_| Error::Unavailable)
    }
}
