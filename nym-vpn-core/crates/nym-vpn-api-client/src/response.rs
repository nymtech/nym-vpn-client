...
mod tests {
    ...
    #[test]
    fn nym_vpn_subscription_kind_parsing() {
        let parse = |raw: &str| {
            serde_json::from_str::<NymVpnSubscriptionKind>(&format!("\"{raw}\"")).unwrap()
        };
        assert_eq!(parse("one_month"), NymVpnSubscriptionKind::OneMonth);
        assert_eq!(parse("six_months"), NymVpnSubscriptionKind::SixMonths);
        assert_eq!(parse("two_years"), NymVpnSubscriptionKind::TwoYears);
        assert_eq!(parse("freepass"), NymVpnSubscriptionKind::Freepass);
        assert_eq!(
            parse("nine_months"),
            NymVpnSubscriptionKind::Other("nine_months".to_string())
        );
        assert_eq!(
            serde_json::to_string(&NymVpnSubscriptionKind::SixMonths).unwrap(),
            "\"six_months\""
        );
    }
}
