...
pub fn mock_mnemonic() -> Mnemonic {
    Mnemonic::parse::<&str>("castle afford bone detect know undo prize detect happy recipe enroll primary finish acoustic banana airport conduct ozone enhance winner pen retreat tip search").unwrap()
}

pub fn mock_vpn_account() -> VpnApiAccount {
    VpnApiAccount::try_from(mock_mnemonic()).unwrap()
}
...
pub async fn mock_account_controller() -> TestBench {
    // Setup storage
    ...
    let vpn_api_server = MockServer::start();
    let nym_vpn_api_client = VpnApiClient::new(
        vpn_api_server.base_url().parse().unwrap(),
        mock_user_agent(),
    )
    .unwrap();
