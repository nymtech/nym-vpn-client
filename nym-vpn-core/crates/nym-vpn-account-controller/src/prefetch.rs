...
//!
//! Caller invariant: this opens credential storage at `data_dir`; it must not
//! run while a controller (in-process or in the network extension) owns the
//! same path. See the Apple prefetch spec (temporal exclusion).
