...
impl NymVpnService {
    ...
    async fn handle_service_command(&mut self, command: VpnServiceCommand) {
        match command {
            ...
            VpnServiceCommand::SubscribeToTunnelState(tx, ()) => {
                let rx = self.handle_subscribe_to_tunnel_state().await;
                let _ = tx.send(rx);
            }
            ...
            VpnServiceCommand::SubscribeToAccountControllerState(tx, ()) => {
                let rx = self.handle_subscribe_to_account_controller_state().await;
                let _ = tx.send(rx);
            }
    ...
    async fn handle_subscribe_to_tunnel_state(&self) -> broadcast::Receiver<TunnelState> {
    ...
    async fn handle_subscribe_to_account_controller_state(
        &self,
