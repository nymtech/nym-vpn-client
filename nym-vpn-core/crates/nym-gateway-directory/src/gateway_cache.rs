...
impl GatewayCacheHandle {
    ...
    pub fn replace_gateway_client(&mut self, gateway_client: GatewayClient) -> Result<()> {
        self.tx
            .send(Command::ReplaceGatewayClient(gateway_client))
            .map_err(|_| Error::Cancelled)
    }
}
...
enum Command {
    ...
    ReplaceGatewayClient(GatewayClient),
}
...
impl GatewayCache {
        ...
        loop {
            tokio::select! {
                ...
                Some(status) = self.connectivity_handle.next() => {
                    tracing::info!("Connectivity changed: {:?}", status);
                    if status.is_online() && !self.is_performed_initial_refresh {
    ...
    fn replace_gateway_client(&mut self, gateway_client: GatewayClient) {
        ...
        // Invalidate cached immediately if gateway performance or performance thresholds change
    ...
    async fn refresh(&mut self, gw_list_types: Vec<GatewayType>, country_types: Vec<GatewayType>) {
        ...
        for gw_type in gw_list_types {
            ...
            tasks.spawn(async move {
                ...
                (gw_type, Either::Right(res))
            });
        }

        while let Some(res) = tasks.join_next().await {
            match res {
                Ok((gw_type, res)) => match res {
