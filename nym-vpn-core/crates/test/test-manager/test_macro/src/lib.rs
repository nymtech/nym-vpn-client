...
pub fn test_function_nym(attributes: TokenStream, code: TokenStream) -> TokenStream {
    ...
    let register_test = create_test_nym(test_function);
...
fn create_test_nym(test_function: TestFunction) -> proc_macro2::TokenStream {
    ...
    let wrapper_closure = quote! {
        ...
        rpc: test_rpc::NymServiceClient,
        nym_client: Option<::nym_vpn_proto::rpc_client::RpcClient>|
        {
            let nym_client = nym_client.expect("Test functions defined using the macro should be given a Nym client");
            Box::pin(async move {
                #func_name(test_context, rpc, nym_client).await.map_err(Into::into)
            })
        }
    };
