...
pub async fn run(config: &Config, vm_config: &VmConfig) -> Result<QemuInstance> {
    ...
    qemu_cmd.args([
        ...
        "-serial",
        "pty",
        // additional serial port is required because the first one
        // is already taken up by getty login prompt
        // /dev/ttyS1
        "-serial",
        "pty",
        // TODO only for testing purposes, we allocate another one
        // this one (last one defined) will get picked up by Mullvad daemon
        "-serial",
        "pty",
    ...
    //         // in an interactive session, you'd typically redirect  guest's serial
    //         // port to host's stdio
    //         // however, this isn't needed here because we already redirect it to pty
    //         // qemu_cmd.args(["-serial", "mon:stdio"]);
    //     }
