...
/// Timeout of [ConnCheckerHandleNym::check_connection].
...
// TODO dz redefine for nym
// #[macro_export]
// macro_rules! assert_tunnel_state_nym {
...
pub async fn install_app(
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            if rpc.nymvpn_daemon_get_status().await? == ServiceStatus::Running {
                return Ok::<_, Error>(());
            }
...
pub fn get_package_desc(name: &str) -> Package {
    Package {
        path: Path::new(&TEST_CONFIG_NYM.artifacts_dir).join(name),
    }
}
...
pub async fn restart_daemon_with<K, V, Env>(
    ...
    _: NymProxyClient, // Just consume the old proxy client
    env: Env,
    ...
    V: Into<String>,
{
    ...
    Ok(test_context.rpc_provider.new_client_nym().await)
}
...
//     loop {
//         let result = rpc
//             .geoip_lookup(TEST_CONFIG_NYM.mullvad_host.clone())
//             .await
...
pub async fn get_app_env() -> anyhow::Result<HashMap<String, String>> {
    use crate::tests::envs::mainnet;

    Ok(HashMap::from_iter(vec![
        (
            mainnet::BECH32_PREFIX_VAR.to_string(),
            mainnet::BECH32_PREFIX.to_string(),
        ),
        (
            mainnet::MIX_DENOM_VAR.to_string(),
            mainnet::MIX_DENOM.to_string(),
        ),
        (
            mainnet::MIX_DENOM_DISPLAY_VAR.to_string(),
            mainnet::MIX_DENOM_DISPLAY.to_string(),
        ),
        (
            mainnet::STAKE_DENOM_VAR.to_string(),
            mainnet::STAKE_DENOM.to_string(),
        ),
        (
            mainnet::STAKE_DENOM_DISPLAY_VAR.to_string(),
            mainnet::STAKE_DENOM_DISPLAY.to_string(),
        ),
        (
            mainnet::DENOMS_EXPONENT_VAR.to_string(),
            mainnet::DENOMS_EXPONENT.to_string(),
        ),
        (
            mainnet::MIXNET_CONTRACT_ADDRESS_VAR.to_string(),
            mainnet::MIXNET_CONTRACT_ADDRESS.to_string(),
        ),
        (
            mainnet::VESTING_CONTRACT_ADDRESS_VAR.to_string(),
            mainnet::VESTING_CONTRACT_ADDRESS.to_string(),
        ),
        (
            mainnet::GROUP_CONTRACT_ADDRESS_VAR.to_string(),
            mainnet::GROUP_CONTRACT_ADDRESS.to_string(),
        ),
        (
            mainnet::ECASH_CONTRACT_ADDRESS_VAR.to_string(),
            mainnet::ECASH_CONTRACT_ADDRESS.to_string(),
        ),
        (
            mainnet::MULTISIG_CONTRACT_ADDRESS_VAR.to_string(),
            mainnet::MULTISIG_CONTRACT_ADDRESS.to_string(),
        ),
        (
            mainnet::COCONUT_DKG_CONTRACT_ADDRESS_VAR.to_string(),
            mainnet::COCONUT_DKG_CONTRACT_ADDRESS.to_string(),
        ),
        (
            mainnet::REWARDING_VALIDATOR_ADDRESS_VAR.to_string(),
            mainnet::REWARDING_VALIDATOR_ADDRESS.to_string(),
        ),
        (
            mainnet::STATISTICS_SERVICE_DOMAIN_ADDRESS_VAR.to_string(),
            mainnet::STATISTICS_SERVICE_DOMAIN_ADDRESS.to_string(),
        ),
        (mainnet::NYXD_VAR.to_string(), mainnet::NYXD.to_string()),
        (
            mainnet::NYM_API_VAR.to_string(),
            mainnet::NYM_API.to_string(),
        ),
        (
            mainnet::NYXD_WS_VAR.to_string(),
            mainnet::NYXD_WS.to_string(),
        ),
        (
            mainnet::NYM_VPN_API_VAR.to_string(),
            mainnet::NYM_VPN_API.to_string(),
        ),
    ]))
}
...
//         &get_query_from_current_settings(mullvad_client).await?,
//         &relay_list,
...
pub struct ConnCheckerHandleNym<'a> {
    checker: &'a mut ConnCheckerNym,
...
//         leak_destination: SocketAddr,
//     ) -> Self {
//         let artifacts_dir = &TEST_CONFIG_NYM.artifacts_dir;
//         let executable_path = match TEST_CONFIG_NYM.os {
...
//                 "--leak-icmp",
//                 "--url",
//                 &format!("https://am.i.{}/json", TEST_CONFIG_NYM.mullvad_host),
//             ]
//             .map(String::from)
//             .to_vec();
//
...
//
//         if self.split && TEST_CONFIG_NYM.os == Os::Linux {
//             self.mullvad_client
...
//     pub async fn split(&mut self) -> anyhow::Result<()> {
//         if TEST_CONFIG_NYM.os == Os::Linux {
//             self.checker
//                 .mullvad_client
