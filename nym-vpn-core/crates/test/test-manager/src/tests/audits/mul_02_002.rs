...
//!
//! Fail to leak traffic to verify that mitigation "Firewall allows deanonymization by eavesdropper"
//! works.
//!
//! # Vulnerability
//! 1. Connect to a relay on port 443. Record this relay's IP address (the new gateway of the
//!    client)
//! 2. Start listening for unencrypted traffic on the outbound network interface (Choose some
//!    human-readable, identifiable payload to look for in the outgoing TCP packets)
...
use crate::{
    network_monitor::{MonitorOptions, ParsedPacket, start_packet_monitor},
    tests::{
        TestContext,
        helpers::{ConnChecker, connect_and_wait, constrain_to_relay, disconnect_and_wait},
    },
};
