...
if [[ $# -eq 0 ]]; then
    ...
    ios_pattern='nym-vpn-core-v[0-9]+\.[0-9]+\.[0-9]+-beta\.[0-9]{12}_ios_universal\.zip'
    macos_pattern='nym-vpn-core-v[0-9]+\.[0-9]+\.[0-9]+-beta\.[0-9]{12}_macos_universal\.tar\.gz'
fi
...
LIB_VERSION=$(echo "$ios_asset" | sed -E "s/nym-vpn-core-v([0-9]+\.[0-9]+\.[0-9]+-(dev|beta)\.[0-9]{12})_ios_universal\.zip/\1/")
