...
class SmokeTest {
    ...
    private fun UiDevice.connect() {
        // Open a show from one of the carousels
        runAction(By.res(Constants.CONNECT_TEST_TAG)) { click() }
        waitForIdle()
    }

    private fun UiDevice.disconnect() {
        runAction(By.res(Constants.DISCONNECT_TEST_TAG)) { click() }
        waitForObject(By.res(Constants.CONNECT_TEST_TAG), 10L.seconds)
        waitForIdle()
    }

    private fun UiDevice.connectWithPermissions() {
        runAction(By.res(Constants.CONNECT_TEST_TAG)) { click() }
        waitForObject(By.res(Constants.DISCONNECT_TEST_TAG), 10L.seconds)
        waitForIdle()
    }
