...
sealed class Event(val message: StringValue = StringValue.Empty){
    sealed class Error(message: StringValue = StringValue.Empty) : Event(message) {
        ...
        data object LoginFailed : Error(StringValue.StringResource(R.string.credential_failed_message))
        data class Exception(val exception: kotlin.Exception) : Error(exception.message?.let { StringValue.DynamicString(it) }
