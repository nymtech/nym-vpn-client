...
### Fixed
- Bind the VPN process to the physical network only while the cover TUN is up (registration). Unbind on the data TUN so in-tunnel metadata at 10.1.0.1 can complete on kernels that ignore uid-exclude (https://github.com/nymtech/nym-vpn-client/pull/6333)
