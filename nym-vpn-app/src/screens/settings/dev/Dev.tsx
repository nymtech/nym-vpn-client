...
function Dev() {
  ...
  const mixnetData = (data: MixnetData) => (
    <div data-testid="dev-mixnet-data">
      <h3 className="text-lg mb-2" data-testid="dev-mixnet-title">Mixnet data</h3>
      <div className="bg-black/20 rounded-md flex flex-col gap-3 font-mono p-2 overflow-x-scroll" data-testid="dev-mixnet-details">
        <div className="select-text cursor-text">
          {data.nymAddress && (
            <>
              {'nym address:'}
              <div data-testid="dev-mixnet-nym-address">{data.nymAddress?.nymAddress}</div>
            </>
          )}
          {data.exitIpr && (
            <>
              {'exit ipr:'}
              <div data-testid="dev-mixnet-exit-ipr">{data.exitIpr?.nymAddress}</div>
            </>
          )}
  ...
  const wgNode = (node: WgNode, nodeType: 'entry' | 'exit') => (
    <div className="select-text cursor-text" data-testid={`dev-wg-${nodeType}-node`}>
      <div data-testid={`dev-wg-${nodeType}-endpoint`}>{`endpoint: ${node.endpoint}`}</div>
      <div data-testid={`dev-wg-${nodeType}-ipv4`}>{`private ipv4: ${node.privateIpv4}`}</div>
      <div data-testid={`dev-wg-${nodeType}-ipv6`}>{`private ipv6: ${node.privateIpv6}`}</div>
      {'pub key:'}
  ...
  const wgData = (data: WireguardData) => (
    <div data-testid="dev-wg-data">
      <h3 className="text-lg mb-2" data-testid="dev-wg-title">Wg data</h3>
      <div className="bg-black/20 rounded-md flex flex-col gap-3 font-mono p-2 overflow-x-scroll" data-testid="dev-wg-details">
        entry:
  ...
  return (
    <PageAnim className="h-full flex flex-col py-6 gap-6 select-none cursor-default" data-testid="dev-page">
      <SettingsMenuCard
      ...
      <div data-testid="dev-state-container">
        <h3 className="text-lg mb-2" data-testid="dev-state-title">State</h3>
        <div 
      ...
      {tunnel && (
        <div data-testid="dev-tunnel-container">
          <h3 className="text-lg mb-2" data-testid="dev-tunnel-title">Tunnel</h3>
          <div 
