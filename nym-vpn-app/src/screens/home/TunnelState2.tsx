...
type Phase = 'disconnected' | 'connecting' | 'connected' | 'canceling' | 'error';
...
export function TunnelState2() {
  ...
  // ─── Glow color ───────────────────────────────────────────────────────────
  const glowColor = isMixnet ? 'rgba(180,182,186,0.55)' : 'rgba(91,240,160,0.70)';
  ...
  const getError = () => {
    if (state === 'offline')
      return <p>{t('offline-message')}</p>;
    if (state === 'offline-auto-reconnect')
      return <p>{t('offline-reconnect-message')}</p>;
    if (tunnelError)
      return (
        <p data-testid="tunnel-specific-error">{tTE(tunnelError)}</p>
      );
    if (isAccountError) {
  ...
  return (
    <div className="inline-flex flex-col items-center gap-2 rounded-lg p-3.5 h-full justify-center">
      ...
      <div className="flex h-4 items-center justify-center">
        ...
        {phase === 'error' && (
          <div className="text-[9px] text-error text-center">
            {getError()}
          </div>
        )}
      </div>
